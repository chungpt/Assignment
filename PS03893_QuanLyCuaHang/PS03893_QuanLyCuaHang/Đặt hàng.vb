...
Public Class frmDangKi
    ...
    Private Sub btnXacNhanDK_Click(sender As Object, e As EventArgs) Handles btnXacNhanDK.Click
        'khai báo biến
        Dim conn As SqlConnection = New SqlConnection(chuoiketnoi)
        Dim query As String = "Insert Into KHACHHANG values (@MAKH,@TENKH,@NGAYSINH,@DIACHI,@SDT) Insert Into HOADON values (@MAHD,@MAKH,@NGAYLAP) Insert Into CHITIETHD values (@MAHD,@MASP,@NGAYMUA,@NGAYGIAOHANG) "
        Dim query2 As String = "Insert Into HOADON values (@MAHD,@MAKH,@NGAYLAP) Insert Into CHITIETHD values (@MAHD,@MASP,@NGAYMUA,@NGAYGIAOHANG) "
            ...
            Try
                ...
                Else
                    ...
                    Else
                        ...
                        save2.Parameters.AddWithValue("@MASP", lblHienThiMaSach.Text)
                        save2.Parameters.AddWithValue("@NGAYMUA", dtbNgayDK.Text)
                        save2.Parameters.AddWithValue("@NGAYGIAOHANG", dtbNgayTra.Text)
                        ...
                        Else
                            ...
                            Else
                                ...
                                Else
                                    ...
                                    save.Parameters.AddWithValue("@MASP", lblHienThiMaSach.Text)
                                    save.Parameters.AddWithValue("@NGAYMUA", dtbNgayDK.Text)
                                    save.Parameters.AddWithValue("@NGAYGIAOHANG", dtbNgayTra.Text)
                                    save.ExecuteNonQuery()
                                    conn.Close() 'đóng kết nối
                                    MessageBox.Show("Đặt hàng thành công, nhân viên của chúng tôi sẽ gọi cho bạn sớm nhất")
                                    txtHD.Text = "HD"
                                    txtMaKH.Text = "KH"
                                End If
                            End If
    ...
    Private Sub dtbNgayTra_ValueChanged(sender As Object, e As EventArgs) Handles dtbNgayTra.ValueChanged
        If dtbNgayTra.Value < dtbNgayDK.Value Or dtbNgayTra.Value = dtbNgayDK.Value Then
            MessageBox.Show("Ngày giao hàng phải sau ngày bán", "Lỗi", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error)
            dtbNgayTra.Value = dtbNgayDK.Value.AddDays(2)
        End If
    End Sub
